document.addEventListener("DOMContentLoaded", function () {
    ...
    // -----------------------------
    // Animazione clessidra su pagina eventi.html
    // -----------------------------
    if (window.location.pathname.includes("eventi.html")) {
        const loadingSpinner = document.createElement('div');
        loadingSpinner.classList.add('loading-spinner');
        document.body.appendChild(loadingSpinner);

        // Aggiungi una clessidra con animazione CSS
        setTimeout(() => {
            loadingSpinner.remove(); // Rimuove la clessidra dopo 2 secondi
        }, 2000);
    }
});
